...
export  class TeslaBatteryComponent implements OnInit {
  ...
  constructor(public fb: FormBuilder, public batteryService: BatteryService) {}
  ...
  ngOnInit() {
    ...
    this.stats = this.calculatesStats(this.results, this.tesla.controls['config'].value);

    this.tesla.controls['config'].valueChanges.subscribe(data => {
      this.stats = this.calculatesStats(this.results, data);
    });
  }

  private calculatesStats(models, value): Stat[] {
